...
class LinearSuper(nn.Linear):
    ...
    def _sample_parameters(self):
        self.samples['weight'] = self.sample_weight(self.weight, self.sample_in_dim, self.sample_out_dim)
        self.samples['bias'] = self.bias
        if self.bias is not None:
            self.samples['bias'] = self.sample_bias(self.bias, self.sample_out_dim)
    ...
    def get_complexity(self, sequence_length):
        total_flops = 0
        total_flops += sequence_length  * self.samples['bias'].size(0)
